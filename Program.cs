...
namespace AtomApp
{
    class Program
    {
        ...
        const string FIRST_FILE = "first-atom.json";
        const string ALL_FILE = "all-atoms.json";

        //test harness
        static void AddAtoms()
        {
            //add the following statement to a class method
        ...
        {
            TextReader reader = new StreamReader(FIRST_FILE);

            Atom a = JsonSerializer.Deserialize<Atom>(reader.ReadToEnd());

            Console.WriteLine("--- ATOM READ FROM FILE:");
            Console.WriteLine(a.ToString());
        ...
        static void SaveAll()
        {
            TextWriter writer = new StreamWriter(ALL_FILE);
        ...
        static void ReadAll()
        {
            TextReader reader = new StreamReader(ALL_FILE);

            string line = reader.ReadLine();

            Console.WriteLine("--- ATOM READ ALL FROM FILE:");

            while (line != null)
            {
                Atom a = JsonSerializer.Deserialize<Atom>(line);
